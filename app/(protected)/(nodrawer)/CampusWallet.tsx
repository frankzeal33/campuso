...
function TransactionRow({
  ...
  return (
    <View className="flex-row items-center py-2.5">
      <View
        className={`size-11 items-center justify-center rounded-full ${credit ? "bg-green-drawer" : "bg-yellow-light"}`}
      >
        <Ionicons
          name={credit ? "arrow-down" : "arrow-up"}
          size={19}
          color={credit ? "#008751" : "#8A5A00"}
        />
      </View>
      <View
      ...
      >
        <View className="flex-row items-center justify-between">
          ...
          <Text
            className={`font-msbold text-sm ${credit ? "text-green" : "text-gray"}`}
          >
            {credit ? "+" : "-"}
            {money(item.amount)}
          </Text>
...
export default function CampusWallet() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <AppScreenHeader
        title="Campus wallet"
        subtitle="Fund, pay and track your money"
        back
      />
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <View className="mb-4 overflow-hidden rounded-3xl bg-green p-5">
              <View className="flex-row items-center justify-between">
                <View className="size-11 items-center justify-center rounded-full bg-white/15">
                  <Ionicons name="wallet-outline" size={23} color="#fff" />
                </View>
                <Ionicons
              ...
              <Text className="mt-6 font-mregular text-xs text-white/70">
                Available balance
              </Text>
              <Text className="mt-1 font-mbold text-3xl text-white">
                {money(wallet.total)}
              </Text>
              <Text className="mt-2 font-mregular text-[10px] text-white/60">
                Secure campus payments in one place
              </Text>
              <Pressable
                onPress={() =>
                  router.push("/(protected)/(nodrawer)/FundWallet" as any)
                }
              ...
              >
                ...
                <Text className="ml-2 font-msbold text-sm text-green">
                  Fund wallet
                </Text>
              </Pressable>
            </View>
