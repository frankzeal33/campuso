...
function RewardRow({
  ...
  return (
    <View className="flex-row items-center py-2.5">
      <View
        className={`size-11 items-center justify-center rounded-full ${earned ? "bg-green-drawer" : "bg-yellow-light"}`}
      >
        <Ionicons
          name={earned ? "gift-outline" : "time-outline"}
          size={19}
          color={earned ? "#008751" : "#8A5A00"}
        />
      </View>
      <View
        className={`ml-3 flex-1 pb-3.5 ${isLast ? "" : "border-b border-gray-100"}`}
      >
        <View className="flex-row justify-between">
          ...
          <Text
            className={`font-msbold text-sm ${earned ? "text-green" : "text-gray-300"}`}
          >
            +{item.points} pts
          </Text>
        </View>
        <View className="mt-1 flex-row justify-between">
          <Text className="font-mregular text-[9px] capitalize text-gray-300">
            {item.status}
          </Text>
...
export default function ReferralWallet() {
  ...
  const invite = () =>
    Share.share({
      message: `Join me on Campuso with referral code ${refData.referralCode}. Connect with your campus community and earn rewards.`,
    });
  return (
          ...
          <View>
              ...
              <View className="flex-row items-start justify-between">
                <View>
                  ...
                  <Text className="mt-1 font-mbold text-3xl text-white">
                    {refData.totalEarned} pts
                  </Text>
                </View>
                <View className="size-12 items-center justify-center rounded-full bg-yellow">
                  <Ionicons name="gift-outline" size={24} color="#2F2F2F" />
                </View>
              </View>
              <View className="mt-5 rounded-2xl bg-white/10 p-4">
                <Text className="font-mregular text-[10px] text-white/60">
                  YOUR REFERRAL CODE
                </Text>
                <View className="mt-1 flex-row items-center justify-between">
                  <Text className="font-mbold text-lg tracking-wider text-white">
            ...
            <View className="mb-4 flex-row gap-3">
              <View className="flex-1 rounded-2xl bg-green-drawer p-4">
                <Text className="font-mbold text-xl text-green">
                  {refData.invitees}
                </Text>
              ...
              <View className="flex-1 rounded-2xl bg-yellow-light p-4">
                <Text className="font-mbold text-xl text-gray">20 pts</Text>
                <Text className="mt-1 font-mregular text-[10px] text-gray-300">
                  Next reward
                </Text>
              </View>
            ...
            <CustomButton
              ...
              handlePress={invite}
              containerStyles="rounded-2xl mb-6"
              textStyles="text-white text-sm"
            />
            <Text className="font-mbold text-lg text-gray">Reward history</Text>
            <Text className="mb-1 mt-0.5 font-mregular text-[10px] text-gray-300">
            ...
            >
              {(["all", "earned", "pending"] as const).map((item) => {
                ...
                return (
                  ...
                  >
                    <Text
                      className={`font-msbold text-[10px] capitalize ${active ? "text-white" : "text-gray-300"}`}
                    >
                      {item}
                    </Text>
                  </Pressable>
        ...
        ListEmptyComponent={
          <View className="items-center py-10">
            <View className="size-12 items-center justify-center rounded-full bg-gray-light">
              <Ionicons name="gift-outline" size={22} color="#787878" />
            </View>
            <Text className="mt-3 font-msbold text-sm text-gray">
              No rewards here
            </Text>
            <Text className="mt-1 font-mregular text-[10px] text-gray-300">
              Try another reward filter.
            </Text>
          </View>
        }
      />
