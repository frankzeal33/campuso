  ...
  return (
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
      <ProviderScreenHeader
        ...
        subtitle="Respond quickly to student requests"
      />
      <ScrollView
      ...
      >
        ...
        >
          {["All", "New", "In progress", "Completed"].map((item) => (
            <Pressable
              ...
              className={`rounded-full px-4 py-2.5 ${filter === item ? "bg-green" : "bg-gray-light"}`}
            >
              <Text
        ...
        <View className="px-4">
          {visible.map((item) => (
            <View key={item.id} className="mb-3 rounded-2xl bg-gray-light p-4">
              <View className="flex-row items-start">
                <View className="size-11 items-center justify-center rounded-full bg-white">
                  <Ionicons name="receipt-outline" size={20} color="#008751" />
                </View>
                <View className="ml-3 flex-1">
                  ...
                  <Text className="mt-2 font-mregular text-[8px] text-gray-300">
                    {item.time}
                  </Text>
                </View>
              </View>
              <View className="mt-3 flex-row items-center justify-between border-t border-gray-100 pt-3">
                <Text className="rounded-full bg-white px-3 py-1.5 font-msbold text-[8px] text-gray">
                  {item.status}
                </Text>
                <View className="flex-row gap-2">
                  <Pressable className="rounded-xl border border-green px-3 py-2">
                    <Text className="font-msbold text-[9px] text-green">
                      Message
                    </Text>
                  </Pressable>
                  <Pressable className="rounded-xl bg-green px-3 py-2">
