...
export default function EditProfile() {
  ...
  const avatar = form.profilePicture
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <AppScreenHeader
        title="Edit profile"
        subtitle="Keep your campus identity up to date"
        back
      />
      <KeyboardAvoidingView
        className="flex-1"
        ...
        >
          <View className="mb-6 items-center rounded-3xl bg-green-drawer p-5">
            <Pressable onPress={pickPhoto} className="relative">
              <Image source={avatar} className="size-24 rounded-full" />
              <View className="absolute bottom-0 right-0 size-8 items-center justify-center rounded-full border-2 border-green-drawer bg-green">
            ...
            <Text className="mt-1 font-mregular text-[9px] text-gray-300">
              Tap the photo to choose another
            </Text>
          </View>
          <View className="gap-4">
            <FormField
              title="Full name"
              value={form.fullName}
              handleChangeText={(value: string) => update("fullName", value)}
            ...
            <FormField
              title="Username"
              value={form.userName}
              handleChangeText={(value: string) => update("userName", value)}
            ...
            <FormField
              title="Email"
              value={form.email}
              handleChangeText={(value: string) => update("email", value)}
            ...
            <FormField
              ...
              handleChangeText={(value: string) => update("phoneNumber", value)}
            ...
            <FormField
              title="School"
              value={form.school}
              handleChangeText={(value: string) => update("school", value)}
            ...
            <View className="flex-row gap-3">
              <View className="flex-1">
                <FormField
                  title="Course"
                  value={form.course}
                  handleChangeText={(value: string) => update("course", value)}
                  placeholder="Course"
                  labelStyle="text-xs text-gray"
                  inputContainerStyles="rounded-2xl border-0"
                  inputStyles="text-sm"
              ...
              <View className="w-24">
                <FormField
                  title="Level"
                  value={form.level}
                  handleChangeText={(value: string) => update("level", value)}
                  placeholder="400L"
                  labelStyle="text-xs text-gray"
                  inputContainerStyles="rounded-2xl border-0"
                  inputStyles="text-sm"
            ...
            <TextArea
              title="Bio"
              value={form.bio}
              handleChangeText={(value: string) => update("bio", value)}
              placeholder="Tell your campus community about yourself"
              labelStyle="text-xs text-gray"
              inputContainerStyles="h-28 rounded-2xl border-gray-200"
              inputStyles="h-24 text-sm"
              maxLength={180}
            />
            <FormField
              title="Interests"
              value={form.interests}
              handleChangeText={(value: string) => update("interests", value)}
              placeholder="Technology, Music, Sports"
              labelStyle="text-xs text-gray"
              inputContainerStyles="rounded-2xl border-0"
              inputStyles="text-sm"
            />
            <Text className="-mt-2 font-mregular text-[9px] text-gray-300">
