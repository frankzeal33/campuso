...
const FormField = ({ title, value, placeholder, inputBg, keyboardType, handleChangeText, disabled, labelStyle, otherStyles, ...props}: formProps) => {
    ...
    <View className={`space-y-2 ${otherStyles}`}>
      {title ? <Text className={`text-base font-msbold pb-2 ${labelStyle ? labelStyle : 'text-green'}`}>{title}</Text> : ''}
      <View className={`${inputBg ? inputBg : 'bg-inputBg'} border border-inputBg w-full h-[52px] px-4 rounded-md focus:border-orange-300 items-center flex-row gap-1`}>
